...
const exampleSelector = await LengthBasedExampleSelector.fromExamples(
  examples,
  {
    examplePrompt, // 示例提示词模板
    maxLength: 700, // 最大示例长度，单位为字符
    getTextLength: (text) => text.length, // 计算文本长度的函数
  },
);
...
// 测试用例
const current_requirement =
  "我们本周在做「内部 AI 助手」项目，既有稳定性保障（处理线上问题），" +
  "也有新功能上线（接入知识库、日志检索）。希望周报既能体现「把坑都兜住了」，" +
  "又能展示一部分业务侧能感知到的亮点。";

const fewShotPromptFormatted = await fewShotPrompt.format({
  current_requirement,
});

console.log("\n=== FewShotPromptTemplate 组合后的 Prompt：");
console.log(fewShotPromptFormatted);
